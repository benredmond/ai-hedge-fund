        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                assert len(tools.tools) >= 5, (
                    f"Expected >=5 tools, got {len(tools.tools)}"
                )
                ...
                assert any(
                    ...
                    or "price" in name.lower()
                    for name in tool_names
                ), f"Expected stock/yahoo tools, got: {tool_names}"

                print(
                    f"✅ yfinance MCP operational with {len(tools.tools)} tools: {tool_names}"
                )
