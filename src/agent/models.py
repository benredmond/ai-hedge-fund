...
class Strategy(BaseModel):
    ...
    def convert_weights_to_dict(cls, v, info: ValidationInfo):
        ...
        if isinstance(v, list):
            ...
            if len(v) != len(assets):
                raise ValueError(f"Weights list length ({len(v)}) must match assets length ({len(assets)})")
...
class EdgeScorecard(BaseModel):
    ...
    def total_score(self) -> float:
        ...
        return (self.specificity + self.structural_basis +
                self.regime_alignment + self.differentiation +
                self.failure_clarity + self.mental_model_coherence) / 6

    @field_validator('specificity', 'structural_basis', 'regime_alignment',
                     'differentiation', 'failure_clarity', 'mental_model_coherence')
    @classmethod
